...
void CommandHandler::handleCommand(const std::string &command, ChatTab *tab)
{
	...
	std::string args(command, pos == std::string::npos ? command.size() : pos
			+ 1);
	...
	else if (type == "setidleinterval")
	{
		handleIdleInterval(args, tab);
	}
	else if (type == "setidlemessage")
	{
		handleIdleMessage(args, tab);
	}
	else if (type == "setuseidle")
	{
		handleUseIdle(args, tab);
	}
	// My Commands End
	else
	{
...
char CommandHandler::parseBoolean(const std::string &value)
{
	...
	if (opt == "1" || opt == "y" || opt == "Y" || opt == "t" || opt == "T")
		return 1;
	else if (opt == "0" || opt == "n" || opt == "N" || opt == "f" || opt == "F")
		return 0;
	else
		return -1;
}
...
void CommandHandler::handleHelp(const std::string &args, ChatTab *tab)
{
	if (args == "")
	{
		...
		tab->chatLog(
				_("/query > Makes a tab for private messages with another user"));
		...
		tab->chatLog(
				_("/record > Start recording the chat to an external file"));
		tab->chatLog(
				_("/toggle > Determine whether <return> toggles the chat log"));
		tab->chatLog(
	...
	else if (args == "help") // Do this before tabs so they can't change it
	{
		...
		tab->chatLog(
				_("This command displays a list of all commands available."));
	...
	else if (args == "present")
	{
		...
		tab->chatLog(
				_("This command gets a list of players within hearing and "
	...
	else if (args == "record")
	{
		...
		tab->chatLog(
				_("This command starts recording the chat log to the file "
						"<filename>."));
	...
	else if (args == "toggle")
	{
		...
		tab->chatLog(
				_("This command sets whether the return key should toggle the "
...
void CommandHandler::handleWhere(const std::string &args, ChatTab *tab)
{
	...
	where << map_path << ", coordinates: " << ((player_node->getPixelX() - 16)
			/ 32) << ", " << ((player_node->getPixelY() - 32) / 32);
...
void CommandHandler::handleToggle(const std::string &args, ChatTab *tab)
{
	...
	{
		tab->chatLog(
				chatWindow->getReturnTogglesChat() ? _("Return toggles chat.")
...
void CommandHandler::handleSetTop(const std::string &args, ChatTab *tab)
{
	...
	msg << "Square 'top' setted to: " << (int) player_node->mY;
...
void CommandHandler::handleSetBottom(const std::string &args, ChatTab *tab)
{
	...
	std::stringstream msg;
	msg << "Square 'bottom' setted to: " << (int) player_node->mY;
	tab->chatLog(msg.str(), BY_SERVER);
}
void CommandHandler::handleSetLeft(const std::string &args, ChatTab *tab)
{
	...
	std::stringstream msg;
	msg << "Square 'left' setted to: " << (int) player_node->mX;
	tab->chatLog(msg.str(), BY_SERVER);
}
void CommandHandler::handleSetRight(const std::string &args, ChatTab *tab)
{
	...
	std::stringstream msg;
	msg << "Square 'right' setted to: " << (int) player_node->mX;
	tab->chatLog(msg.str(), BY_SERVER);
}
void CommandHandler::handleSetHome(const std::string &args, ChatTab *tab)
{
	...
	std::stringstream msg;
	msg << "Home point setted to: " << (int) player_node->mX << ":"
			<< (int) player_node->mY;
	tab->chatLog(msg.str(), BY_SERVER);
}
void CommandHandler::handleIdleInterval(const std::string &args, ChatTab *tab)
{
	char opt;
	if (args.empty())
		return;
...
void CommandHandler::handleIdleMessage(const std::string &args, ChatTab *tab)
{
	char opt;
	if (args.empty())
		return;
	std::stringstream msg;
	msg << "Idle message: " << args;
	tab->chatLog(msg.str(), BY_SERVER);
	player_node->idleMessage = args;
}
void CommandHandler::handleUseIdle(const std::string &args, ChatTab *tab)
{
	char opt;
	if (args.empty())
		opt = 0;
	...
	if (opt == 0)
	{
		tab->chatLog(_("Use idle disabled."), BY_SERVER);
		player_node->useIdle = false;
	}
	else
	{
		tab->chatLog(_("Use idle enabled."), BY_SERVER);
