...
Being *BeingManager::findNearestLivingBeingNotName(Being *aroundBeing, int maxdist,
																   Being::Type type, std::string name) const
{
	...
	{
		Being *being = (*i);
		if(player_node->square && (being->mX < player_node->left || being->mX > player_node->right || being->mY < player_node->top || being->mY > player_node->bottom))
			continue;
#ifdef TMWSERV_SUPPORT
		const Vector &pos = being->getPosition();
		int d = abs(((int) pos.x) - x) + abs(((int) pos.y) - y);
		...
		if ((being->getType() == type || type == Being::UNKNOWN)
			...
			&& being->mAction != Being::DEAD        // no dead beings
			&& being != aroundBeing
			&& ( name == "" || being->getName().find(name,0) == std::string::npos)
			)
		{
			dist = d;
			closestBeing = being;
