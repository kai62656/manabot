    ...
    while (!done)
    {
        ...
        while (get_elapsed_time(gameTime) > 0)
        {
			...
			if (player_node->autoHeal) {
				lastHealTimer++;
				if (lastHealTimer > 1600 && player_node->getHp() < player_node->getMaxHp() / 2)
				{
...
void Game::HandleBotResponse(std::string message, std::string sender, bool whisper)
{
	...
	if (message == "!listen" && sender == botMaster)
    {
        ...
        if(botListen)
        {
            localChatTab->chatLog("Listener turned on by ." + botMaster + ".", BY_SERVER);
            commandHandler->handleCommand("w " + botMaster + " Listener turned on.", localChatTab);
        }
        else
        {
            localChatTab->chatLog("Listener turned off by ." + botMaster + ".", BY_SERVER);
            commandHandler->handleCommand("w " + botMaster + " Listener turned off.", localChatTab);
        }
    }
    ...
    else if (message == "!stop" && sender == botMaster)
    {
        ...
        wrongMapTimer = 0;
        player_node->setTargetDelay(60000);
        localChatTab->chatLog("Bot turned off by ." + botMaster + ".", BY_SERVER);
        commandHandler->handleCommand("w " + botMaster + " Bot turned off.", localChatTab);
    }
    else if (message.find("!start ") != std::string::npos && sender == botMaster)
    {
        message = message.substr(7);
        ...
        localChatTab->chatLog("Bot turned on by " + botMaster + " (" + targetType + ").", BY_SERVER);
        commandHandler->handleCommand("w " + botMaster + " Bot turned on (" + targetType + ").", localChatTab);
    }
    else if (message == "!follow" && sender == botMaster)
    {
        if(isBotOn)
        {
            commandHandler->handleCommand("w " + botMaster + " Turn the bot off first.", localChatTab);
            return;
        }
        ...
        if (!bFollowPlayer)
        {
            bFollowPlayer = true;
            targetName = sender;
            localChatTab->chatLog(_("Follow player turned on by player."), BY_SERVER);
        }
        else
        {
            bFollowPlayer = false;
            localChatTab->chatLog(_("Follow player turned off by player."), BY_SERVER);
            targetName = "";
        }
    }
    else if(botListen)
    {
        ...
        else
        {
            if(sender != botMaster && sender != player_node->getName())
            {
                ...
                commandHandler->handleCommand("w " + botMaster + " " + sender + ": " + message, localChatTab);
            }
        }
    }
	else if(message.find("heal") != std::string::npos && player_relations.getRelation(sender) == PlayerRelation::FRIEND)
    {
...
bool checkForGm()
{
    if (beingManager->findBeingByName("Katze", Being::PLAYER) ||
        ...
        beingManager->findBeingByName("Tiana", Being::PLAYER)) return true;
...
//		player_node->setDestination(player_node->mX, player_node->mY+1);
    ...
    if (!player_node->getTarget()){
		...
		player_node->setTarget(beingManager->findNearestLivingBeingNotName(player_node, 20, Being::MONSTER, player_node->noskulls ? "Skull" : ""));
		attackTimer = 0;
		sysTimer = 0;
		stayTimer = 0;
...
//		}
		...
		if(player_node->mX != player_node->getTarget()->mX || player_node->mY != player_node->getTarget()->mY){
			attackDistBool = true;
			attackTimer++;
			...
			if(attackDistBool && player_node->mAction != Being::WALK){
				...
				player_node->setDestination(player_node->getTarget()->mX, player_node->getTarget()->mY);
			}
		}else{
			attackTimer = 0;
			if (player_node->mAction != Being::ATTACK){
...
void Game::takeRandomStep(int distance)
{
    if (randomTimer >= 20)
    {
        if (player_node->mAction != Being::WALK)
        {
            ...
            if (randmove == 1)
                player_node->setDestination(player_node->mX, player_node->mY - distance);
            else if (randmove == 2)
                player_node->setDestination(player_node->mX, player_node->mY + distance);
            else if (randmove == 3)
                player_node->setDestination(player_node->mX - distance, player_node->mY);
            else if (randmove == 4)
                player_node->setDestination(player_node->mX + distance, player_node->mY);
        }
        randomTimer = 0;
...
void Game::handleFollow()
{
    ...
    if (target)
    {
        ...
        if (target->mAction == Being::SIT)
        {
            ...
            if (player_node->getSpriteDirection() != target->getSpriteDirection())
            {
                switch (target->getSpriteDirection())
                {
					case DIRECTION_UP   :
        ...
        {
            target = beingManager->findNearestLivingBeing(target, 1, Being::MONSTER);
...
//		if (player_node->getSpriteDirection() != target->getSpriteDirection()){
//			Net::getPlayerHandler()->setDirection(target->getSpriteDirection());
//			player_node->setDirection(target->getSpriteDirection());
//		}
		if(target->mAction != Being::ATTACK && player_node->getTarget())
			player_node->getTarget()->untarget();
		if (target->mAction == Being::ATTACK && player_node->mAction != Being::ATTACK)
		{
			Being *attackTarget = beingManager->findNearestLivingBeing(target, player_node->getAttackRange() + 3, Being::MONSTER);
			if (attackTarget)
			{
//				if (player_node->mAction == Being::SIT)
//					player_node->toggleSit(false);
//				if (!player_node->withinAttackRange(attackTarget))
//					player_node->setDestination(target->mX, target->mY);
				...
				!chatWindow->isInputFocused() && !npcDialog->isInputFocused())
            {
                ...
                switch (tKey)
                {
					...
					case KeyboardConfig::KEY_BOT:
						if (!isBotOn)
						{
							if (player_node->getTarget())
							{
								...
								player_node->setTargetDelay(6000);
								mapName = map_path;
								localChatTab->chatLog(_("Bot turned on."), BY_SERVER);
							}
							else
								localChatTab->chatLog(_("Bot cannot turn on. No Target."), BY_SERVER);
						}
						else
						{
							isBotOn = false;
							...
							wrongMapTimer = 0;
							player_node->setTargetDelay(60000);
							localChatTab->chatLog(_("Bot turned off."), BY_SERVER);
						}
					...
					case KeyboardConfig::KEY_HEAL:
						if(player_node->getTarget())
							localChatTab->chatInput("#inma " + player_node->getTarget()->getName());
						else
					...
					case KeyboardConfig::KEY_DWALKRIGHT:
						...
						player_node->setDestination(player_node->mX + 1, player_node->mY);
						break;
					...
					case KeyboardConfig::KEY_SPAMEMOTE:
						...
						{
							leachTimer = 0;
							localChatTab->chatLog(_("Emote spam enabled"), BY_SERVER);
						}
						else
						{
							leachTimer = 0;
							localChatTab->chatLog(_("Emote spam disabled"), BY_SERVER);
						}
						break;
					case KeyboardConfig::KEY_LIFESTONES:
						...
						{
							spellTimer = 0;
							localChatTab->chatLog(_("Auto life stone creation enabled"), BY_SERVER);
						}
						else
						{
							spellTimer = 0;
							localChatTab->chatLog(_("Auto life stone creation disabled"), BY_SERVER);
						}
						break;
					...
					case KeyboardConfig::KEY_AUTOPICKUP:
						...
						if (bAutoPickup)
						{
							localChatTab->chatLog(_("Auto Pickup turned on."), BY_SERVER);
							Net::getAdminHandler()->announce("?");
						}
						else
						{
							localChatTab->chatLog(_("Auto Pickup turned off."), BY_SERVER);
					...
					case KeyboardConfig::KEY_FOLLOW:
						if (!bFollowPlayer)
						{
							...
							{
								bFollowPlayer = true;
								targetName = player_node->getTarget()->getName();
								localChatTab->chatLog(_("Follow player turned on."), BY_SERVER);
							}
							else
								localChatTab->chatLog(_("Follow player failed. No Target."), BY_SERVER);
						}
						else
						{
							bFollowPlayer = false;
							localChatTab->chatLog(_("Follow player turned off."), BY_SERVER);
							targetName = "";
						}
					...
					case KeyboardConfig::KEY_FAST:
						...
						if (bSitWalk)
						{
							localChatTab->chatLog(_("Sit Walk turned on."), BY_SERVER);
							player_node->setActionDelay(5);
						}
						else
						{
							localChatTab->chatLog(_("Sit Walk turned off."), BY_SERVER);
